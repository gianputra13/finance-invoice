...
trait PDF_Signature
{
    public function process_signature()
    {
        ...
        || ($this->type() == 'credit_note') && get_option('show_pdf_signature_credit_note') == 1) {
            ...
            $invoice = $GLOBALS['invoice_pdf'];
            $date = $invoice->date;
            $this->MultiCell(($dimensions['wk'] / 2) - $dimensions['lm'], 0, _l('authorized_signature_text') . ' ' ._l(date('d F Y', strtotime($date))). $blankSignatureLine, 0, 'J', 0, 0, '', '', true, 0, true, true, 0);
